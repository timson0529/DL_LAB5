...
class PrioritizedReplayBuffer:
    ...
    def update_priorities(self, indices, errors):
        ...
        for idx, error in zip(indices, errors):
            self.priorities[idx] = abs(error) + eps
        return
...
class DQNAgent:
    ...
    def get_n_step_transition(self):
        ...
        for idx, (_, _, reward, ns, d) in enumerate(self.n_step_buffer):
        ...
        return state, action, reward_sum, next_state, done

    def run(self, episodes=1000):
        for ep in range(episodes):
            self.n_step_buffer.clear()  #清除n-step buffer
        ...
        #     print("Processed state dtype:", state.dtype)
            ...
            while not done and step_count < self.max_episode_steps:
                ...
                if len(self.n_step_buffer) == self.n_step:
                    ...
                    self.memory.add(n_step_transition, error=None)
                ...
                for s in self.snapshot_steps:
                    if self.env_count >= s and s not in self.saved_snapshot_steps:
                        model_path = os.path.join(self.save_dir, f"task3_{s}.pt")
                        torch.save(self.q_net.state_dict(), model_path)
                        self.saved_snapshot_steps.add(s)
                        print(f"Saved Task 3 snapshot at {s} steps to {model_path}")
            ...
            if ep % 100 == 0:
                ...
                if eval_reward > self.best_reward:
                    ...
                    torch.save(self.q_net.state_dict(), model_path)
                    best_task3_path = os.path.join(self.save_dir, "task3_best.pt")
                    torch.save(self.q_net.state_dict(), best_task3_path)
                    print(f"Saved new best model to {model_path} with reward {eval_reward}")

                print(f"[TrueEval] Ep: {ep} Eval Reward: {eval_reward:.2f} SC: {self.env_count} UC: {self.train_count}")
                if eval_reward >= 19 and self.first_reach_19_step is None:
                    self.first_reach_19_step = self.env_count
                    print(f"First reached score 19 at env step: {self.first_reach_19_step}")
                    wandb.log({"First Reach 19 Step": self.first_reach_19_step, "Eval Reward When First Reach 19": eval_reward})
                wandb.log({"Env Step Count": self.env_count, "Update Count": self.train_count,"Eval Reward": eval_reward})
    ...
    def train(self):
        ...
        loss_each = nn.SmoothL1Loss(reduction="none")(q_values, target_q_values)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb-run-name", type=str, default="task3-pong-enhanced-dqn")
